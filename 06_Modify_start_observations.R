...
# This implies that the start dates of the next observation will be add with 0.00274 (1/365) to give them a slightly later date than the 
...
# modified, in this case the end date will be added with 0.00274. 

# This step is divided in five substeps:

# 18.1 Create OBSERVATIONS_RP data frame
# 18.2 Prepare OBSERVATIONS_RP data frame
# 18.3 The end dates that are the same as the start dates in an observation are added with 0.00274 
# 18.4 The start dates that match the previous end dates are added with 0.00274   
# 18.5 The end dates that are the same as the start dates in an observation are added with 0.00274
...
# the previous End_date is placed in a new column in the next record.  
...
# 18.4 In case Start_date matches previous End_date: Start_date + 0.00274

# In this step each Start_date that matches the previous End_date is added with 0.00274. 
...
# In the first the Start_dates are added with 0.00274.

OBSERVATIONS_RP_CONT_YES <- OBSERVATIONS_RP_BIND_1 %>% 
  filter (Contiguous == "Yes") %>%
  mutate(Start_date = (Start_date + 0.00274))
...
# 18.5 When Start_date == End_date: add 0.00274 to End_date 
...
# 18.6 Create START_OBSERVATIONS_RP and END_OBSERVATIONS_RP data frames
...
# 18.7 Combine and sort the START and END OBSERVATION data frames
